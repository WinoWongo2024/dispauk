...
function renderList() {
  ...
  el.innerHTML = items.map(i =>
    ...
    (i.incidentNumber ? ' · ' + i.incidentNumber : '') +
    (i.imageThumb || i.image ? ' · 📷' : '') +
    '</span></div>' +
...
function clearForm() {
  ...
  if (hint) hint.textContent = 'Photos are auto-compressed: full image for the story page, small thumbnail for the news list (avoids lag).';
}
...
function fillForm(item) {
  ...
  window._imageThumb = item.imageThumb || '';
  const prev = document.getElementById('f-preview');
  if (item.image || item.imageThumb) {
    prev.src = item.image || item.imageThumb;
...
function compressPhoto(dataUrl) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
      function encode(maxW, quality, maxChars) {
      ...
      try {
        const image = encode(1200, 0.72, 140000);
        const imageThumb = encode(480, 0.65, 40000);
        if (image.length > 180000) {
          reject(new Error('Photo is still too large after compress. Try a smaller original.'));
          return;
        }
        resolve({ image, imageThumb, storyKB: Math.round(image.length / 1024), thumbKB: Math.round(imageThumb.length / 1024) });
      } catch (e) {
        reject(e);
      }
    };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (imgInput && !document.getElementById('photo-size-hint')) {
    const hint = document.createElement('p');
    hint.id = 'photo-size-hint';
    hint.className = 'admin-hint';
    hint.textContent = 'Photos are auto-compressed: full image for the story page, small thumbnail for the news list (avoids lag).';
    imgInput.parentNode.insertBefore(hint, imgInput.nextSibling);
  }
  ...
  document.getElementById('f-image').addEventListener('change', e => {
    ...
    if (file.size > 4_000_000) {
      alert('Image file is over 4 MB. Choose a smaller photo.');
      e.target.value = '';
      return;
    }
    const hint = document.getElementById('photo-size-hint');
    if (hint) hint.textContent = 'Compressing photo…';
    const reader = new FileReader();
    reader.onload = async () => {
      try {
        const { image, imageThumb, storyKB, thumbKB } = await compressPhoto(reader.result);
        document.getElementById('f-image-data').value = image;
        window._imageThumb = imageThumb;
        const prev = document.getElementById('f-preview');
        prev.src = image;
        prev.style.display = 'block';
        if (hint) {
          hint.textContent = 'Ready — story image ~' + storyKB + ' KB, list thumbnail ~' + thumbKB + ' KB. Safe for mobile.';
        }
      } catch (err) {
        alert(err.message || 'Could not process image.');
        e.target.value = '';
        if (hint) hint.textContent = 'Photos are auto-compressed for the story page and a small list thumbnail.';
      }
    };
